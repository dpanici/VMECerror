if exist('R_s_vmec,','var') == false
R_s_vmec = eval_series(suvgrid,data.rsmnc,data,'c');
end
...
figure()
title(sprintf('VMEC %s vs s at u=%f, nfp*phi=%f',quant_str,u(u_index),v(v_nfp_index)))
plot(data.phi(s_index:end),quant_vmec(s_index:end,u_index,v_nfp_index))
xlabel('s')
ylabel(sprintf('%s',quant_str))
legend('My Calc','VMEC')
...
title(sprintf('%s vs u at u=%f, s=%f',quant_str,u(u_index),s(s_index)))
xlabel('u')
ylabel(sprintf('%s',quant_str))
...
plot(u,quant_vmec(s_index,:,v_nfp_index))
title(sprintf('VMEC %s vs u at v=%f, s=%f',quant_str,v(v_nfp_index),s(s_index)))
xlabel('u')
ylabel(sprintf('%s',quant_str))
